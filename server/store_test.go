...
func TestStoreSubjectStateConsistency(t *testing.T) {
	testAllStoreAllPermutations(
		...
		func(t *testing.T, fs StreamStore) {
			...
			// Remove first message, will update ss.First.
			removed, err := fs.RemoveMsg(1)
			require_NoError(t, err)
			require_True(t, removed)

			// Should update first, so corrects to 2.
			ss = getSubjectState()
			...
			// Remove last message, will update ss.Last.
			removed, err = fs.RemoveMsg(4)
			require_NoError(t, err)
			require_True(t, removed)

			// Should update last, so corrects to 3.
			ss = getSubjectState()
			...
			// Remove last sequence, will update ss.Last.
			removed, err = fs.RemoveMsg(7)
			require_NoError(t, err)
			require_True(t, removed)

			// Remove first sequence, will update ss.First.
			removed, err = fs.RemoveMsg(3)
			require_NoError(t, err)
			require_True(t, removed)

			// Remove (now) first sequence. Both ss.First and ss.Last should be updated.
